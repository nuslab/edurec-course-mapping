...
def fetch_document(url: str, fetch: Fetcher) -> LinkedDocument:
...
def fetch_documents(
    request: Request, fetch: Fetcher, cache: dict[str, LinkedDocument] | None = None
) -> Request:
    ...
    for url in find_urls(request):
        if url not in cache:
            cache[url] = fetch_document(url, fetch)
...
def scrape(data: Document, fetch: Fetcher, checkpoint: Callable[[Request], None]) -> Document:
